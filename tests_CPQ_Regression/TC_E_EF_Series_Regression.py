...
# Test Metadata
# =========================================================================================================================
# Test Scenario: Regression test for E/EF-Series product
# Test Description: This test script performs regression testing for the E/EF-Series product across different scenarios
#                   including opportunity creation, quote creation, product configuration, order submission and validating
#                   the quote status by approving the quote in TPD.
#                   The test iterates through multiple sets of test data to validate various functionalities and configurations
#                   of the E/EF-Series product in the CPQ system.
# Author: Sunil Reddy
# =========================================================================================================================
...
def test_E_EF_Series_Regression(page: Page, base_url, config, test_case) -> None:  # type: ignore
    lp = LoginPage(page)
    hp = HomePage(page)
    ...
    try:
        ...
        if test_case["Execution"].strip().lower() == "yes":
            ...
            if is_valid_data(test_case["Model_System"]):
                logger.info(
                    f"Testing Iteration Started for the E/EF-Series Model: {test_case["Model_System"]} & Opportunity Type: {test_case["Channel"]}"
                )
            else:
                logger.info(
                    f"Testing Iteration Started for the E/EF-Series Model: {test_case["Sub Product"]} & Opportunity Type: {test_case["Channel"]}"
                )
            ...
            if test_case["Configure Product"].strip().lower() == "yes":
                ...
                if is_valid_data(test_case["Drive Type_Base Storage"]):
                    hpeefs.selectDriveType_BaseStorage(
                        test_case["Drive Type_Base Storage"]
                    )
                    logger.info(
                        f"Selected Drive Type under Base Storage as: {test_case['Drive Type_Base Storage']}"
                    )
                ...
                if is_valid_data(test_case["Select Adapters to Add"]):
                    hpeefs.selectAdaptersToAdd(test_case["Select Adapters to Add"])
                    logger.info(
                        f"Selected Adapters to add as: {test_case['Select Adapters to Add']}"
                    )
            ...
            if test_case["Sold To"].strip().lower() == "yes":
                ...
                if is_valid_data(test_case["Email_Sold To"]):
                    aip.enterSoldTo_2(test_case["Email_Sold To"])
                    logger.info(
                        f"Entered Sold To details: {test_case['Email_Sold To']}"
                    )
            ...
            if test_case["End Customer"].strip().lower() == "yes":
                ...
                if is_valid_data(test_case["Email_End Customer"]):
                    aip.enterEndCustomer_2(test_case["Email_End Customer"])
                    logger.info(
                        f"Entered End Customer details: {test_case['Email_End Customer']}"
                    )

            if test_case["Software Delivery"].strip().lower() == "yes":
                aip.enterSoftwareDelivery()
                logger.info(f"Entered Software Delivery details")

            if test_case["Ship To Customer"].strip().lower() == "yes":
            ...
            if test_case["Override Invalid/LOD Part"].strip().lower() == "yes":
            ...
            if is_valid_data(test_case["Model_System"]):
                logger.info(
                    f"Execution flag is set to 'No'. Skipping the test case for the E/EF-Series Model: {test_case["Model_System"]} & Opportunity Type: {test_case["Channel"]}"
                )
            else:
                logger.info(
                    f"Execution flag is set to 'No'. Skipping the test case for the E/EF-Series Model: {test_case["Sub Product"]} & Opportunity Type: {test_case["Channel"]}"
                )
