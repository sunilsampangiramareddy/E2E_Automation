...
from pages_Software.HomePage_Software import HomePageSoftware
from pages_Switches_Bridges.HomePage_Switches_Bridges import HomePageSwitchesBridges
...
#                FAS/AFF/ASA/AFX(Configure MetroCluster-AFF A20), Switches/Bridges(Cisco Switch),
#                Software(ONTAP Select), submit a quote, and approve the quote in TPD
# Test Description: This test script automates the process of logging into Salesforce, creating an Direct opportunity,
#                   configuring multiple products FAS/AFF/ASA/AFX(Configure MetroCluster-AFF A20),
#                   Switches/Bridges(Cisco Switch), Software(ONTAP Select), updating their details in the
...
relative_file_path = os.path.join("testData/tests_ERP_FTR", "E2E_UAT_018.xlsx")
...
def test_E2E_UAT_018(page: Page, base_url, config, test_case) -> None:
    lp = LoginPage(page)
    hp = HomePage(page)
    ...
    try:
        ...
        # ===============Configure FAS/AFF/ASA/AFX-MetroCluster-(A20) First Product================================================================
        ...
        hpFAS.clickSwitchesTab()
        logger.info(f"Clicked on Switches tab")

        hpFAS.selectIncludeSwitch_Yes()
        logger.info(f"Selected Yes for Include a cluster switch option")

        hpFAS.selectType_Cluster_BackendSwitch(test_case["Type_Cluster/Backend Switch"])
        logger.info(
            f"Selected {test_case['Type_Cluster/Backend Switch']} for Type of Cluster/Backend Switch option"
        )

        hpFAS.selectPortCount_Cluster_BackendSwitch(
            test_case["Port Count_Cluster/Backend Switch"]
        )
        logger.info(
            f"Selected {test_case['Port Count_Cluster/Backend Switch']} for Port Count of Cluster/Backend Switch option"
        )

        hpFAS.selectSpeed_Cluster_BackendSwitch(
            test_case["Speed_Cluster/Backend Switch"]
        )
        logger.info(
            f"Selected {test_case['Speed_Cluster/Backend Switch']} for Speed of Cluster/Backend Switch option"
        )

        hpFAS.selectClusterInterconnectCable_Switch(
            test_case["Cluster Interconnect Cable_Switch"]
        )
        logger.info(
            f"Selected {test_case['Cluster Interconnect Cable_Switch']} for Cluster Interconnect Cable for Switch option"
        )

        hpFAS.selectType_ClusterInterconnectCable_Switch(
            test_case["Type_Cluster Interconnect Cable_Switch"]
        )
        logger.info(
            f"Selected {test_case['Type_Cluster Interconnect Cable_Switch']} for Type of Cluster Interconnect Cable for Switch option"
        )

        hpFAS.selectLength_ClusterInterconnectCable_Switch(
            test_case["Length_Cluster Interconnect Cable_Switch"]
        )
        logger.info(
            f"Selected {test_case['Length_Cluster Interconnect Cable_Switch']} for Length of Cluster Interconnect Cable for Switch option"
        )

        hpFAS.enterQty_ClusterInterconnectCable_Switch(
            test_case["Qty_Cluster Interconnect Cable_Switch"]
        )
        logger.info(
            f"Entered qty: {test_case['Qty_Cluster Interconnect Cable_Switch']} for Cluster Interconnect Cable for Switch option"
        )

        hpFAS.selectWhatCableLengthDoYouWant_Switch(
            test_case["What cable length do you want?_Switch"]
        )
        logger.info(
            f"Selected {test_case['What cable length do you want?_Switch']} for What cable length do you want? option"
        )

        hpFAS.selectResponseTime_ClusterBackendInterconnectSwitchServices(
            test_case["Response Time_Cluster/Backend Switch Services"]
        )
        logger.info(
            f"Selected {test_case['Response Time_Cluster/Backend Switch Services']} for Response Time for Cluster/Backend Switch Services option"
        )

        hpFAS.selectServiceLevel_ClusterBackendInterconnectSwitchServices(
            test_case["Service Level_Cluster/Backend Switch Services"]
        )
        logger.info(
            f"Selected {test_case['Service Level_Cluster/Backend Switch Services']} for Service Level for Cluster/Backend Switch Services option"
        )

        hpFAS.selectTerm_ClusterBackendInterconnectSwitchServices(
            test_case["Term_Cluster/Backend Switch Services"]
        )
        logger.info(
            f"Selected {test_case['Term_Cluster/Backend Switch Services']} for Term for Cluster/Backend Switch Services option"
        )
        ...
        # ===============Configure Switches/Bridges-Cisco Switch-Second Product================================================================
        pp.clickProductsTab()
        logger.info(f"Clicked on Products tab")
        ...
        hpsb = HomePageSwitchesBridges(new_tab)
        logger.info(f"HomePageSwitchesBridges instance created for the new tab")

        if is_valid_data(test_case["Product_2"]):
            hpsb.selectProductName(test_case["Product_2"])
            logger.info(f"Clicked on product: {test_case['Product_2']}")

        if is_valid_data(test_case["Sub Product_2"]):
            hpsb.selectSubProductName(test_case["Sub Product_2"])
            logger.info(f"Clicked on sub-product: {test_case['Sub Product_2']}")

        hpsb.access_SelectAll()
        logger.info(
            f"Accessed select all option under access tab for Switches/Bridges product"
        )

        hpsb.selectManufacturer_Switch(test_case["Manufacturer_Switch_2"])
        logger.info(
            f"Selected manufacturer: {test_case['Manufacturer_Switch_2']} for switch"
        )

        hpsb.clickAgree_PopUp()
        logger.info(f"Clicked on Agree button for Switches/Bridges product")

        hpsb.selectType_Switch(test_case["Type_Switch_2"])
        logger.info(f"Selected type: {test_case['Type_Switch_2']} for switch")

        hpsb.clickAgree_PopUp()
        logger.info(f"Clicked on Agree button for Switches/Bridges product")

        hpsb.selectPorts_Switch(test_case["Ports_Switch_2"])
        logger.info(f"Selected ports: {test_case['Ports_Switch_2']} for switch")

        hpsb.clickAgree_PopUp()
        logger.info(f"Clicked on Agree button for Switches/Bridges product")

        hpsb.selectCountry_Region_PowerAndRackmount(
            test_case["Country/Region_Power and Rackmount_2"]
        )
        logger.info(
            f"Selected country/region for power and rackmount: {test_case['Country/Region_Power and Rackmount_2']} for switch"
        )

        hpsb.clickAgree_PopUp()
        logger.info(f"Clicked on Agree button for Switches/Bridges product")

        hpsb.selectResponseTime_Services(test_case["Response Time_Services_2"])
        logger.info(
            f"Selected response time for services: {test_case['Response Time_Services_2']} for switch"
        )

        hpsb.selectServiceLevel_Services(test_case["Service Level_Services_2"])
        logger.info(
            f"Selected service level for services: {test_case['Service Level_Services_2']} for switch"
        )

        hpsb.selectTerm_Services(test_case["Term_Services_2"])
        logger.info(
            f"Selected term for services: {test_case['Term_Services_2']} for switch"
        )
        ...
        hp.refreshPage()
        logger.info(f"Refreshed the page to avoid any stale element issues")

        hpsb.clickAddToQuote()
        ss.capture_screenshot("Captured Switches/Bridges Product configuration details")
        logger.info(f"Clicked on Add to Quote button for Switches/Bridges product")

        hpc.clickSaveIcon()
        logger.info(f"Clicked on Save button")

        # ===============Configure Software-ONTAP Select-Third Product================================================================
        ...
        hps = HomePageSoftware(new_tab)
        logger.info(f"HomePageSoftware instance created for the new tab")

        if is_valid_data(test_case["Product_3"]):
            hps.selectProductName(test_case["Product_3"])
            logger.info(f"Clicked on product: {test_case['Product_3']}")

        if is_valid_data(test_case["Sub Product_3"]):
            hps.selectSubProductName(test_case["Sub Product_3"])
            logger.info(f"Clicked on sub-product: {test_case['Sub Product_3']}")

        hps.access_SelectAll()
        logger.info(f"Accessed select all option under access tab for Software product")

        hps.selectBuy_Configuration(test_case["Buy_Configuration_3"])
        logger.info(
            f"Selected buy option: {test_case['Buy_Configuration_3']} for Software product"
        )

        hps.enterStandard_Licenses(test_case["Standard_Licenses_3"])
        logger.info(
            f"Entered standard licenses: {test_case['Standard_Licenses_3']} for Software product"
        )

        hps.enterSoftwareSupportTermMonths_Services(
            test_case["Software Support Term Months_Services_3"]
        )
        logger.info(
            f"Entered software support term in months: {test_case['Software Support Term Months_Services_3']} for Software product"
        )

        hps.clickAddToQuote()
        ss.capture_screenshot("Captured Software Product configuration details")
        logger.info(f"Clicked on Add to Quote button for Software product")
