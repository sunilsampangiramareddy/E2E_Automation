...
# Test Metadata
# =========================================================================================================================
# Test Scenario: Verify that a user can configure Quick Ship/Express Pack parts (X65404), submit a quote through B2B XML
#                and validate the quote status in TPD.
# Test Description: This test script automates the process of logging into Salesforce, creating a Indirect opportunity
#                   using Distrubutor, configuring Quick Ship/Express Pack parts (X65404), updating its details in the
#                   Account Information, Initiate Approval Request, GTC Risks, Generate and Export B2B XML for
#                   Quote submission and validating the quote status in TPD.
# Author: Sunil Reddy
# =========================================================================================================================

logger = logging.getLogger("playwright_pytest")
# Load test data from Excel
relative_file_path = os.path.join("testData/tests_ERP_FTR", "E2E_UAT_024.xlsx")
...
def test_E2E_UAT_024(page: Page, base_url, config, test_case) -> None:
    lp = LoginPage(page)
    hp = HomePage(page)
    ...
    try:
        ...
        if is_valid_data(test_case["ProductName PartNumber Or ModelNumber"]):
            pp.searchProductNamePartNumberOrModelNumber(
                test_case["ProductName PartNumber Or ModelNumber"]
            )
            logger.info(
                f"Searched and added ProductName PartNumber or ModelNumber: {test_case['ProductName PartNumber Or ModelNumber']}"
            )
